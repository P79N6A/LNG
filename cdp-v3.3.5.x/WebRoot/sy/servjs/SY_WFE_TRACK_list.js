...
function _ifCheckRadio(){
	//判断是否选中radio
	if(jQuery("input[type='radio'][class='rowIndex']:checked").length == 0){
		alert("请选择流程实例。");
		return;
	}
	return true;
...
figureBtn.bind("click",function() {
	...
	var opts = {"tTitle":"图形化流程跟踪","url":"SY_WFE_TRACK_FIGURE.show.do?data=" + JsonToStr(_viewer.params),"params":_viewer.params,"menuFlag":3};
	Tab.open(opts);
});
