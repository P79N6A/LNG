...
conPwdObj.keyup(function(){
	...
	if(newPswd != conPswd){
		if(conFlag==0){
			...
			jQuery("<div id='conPswd' style='float:left;position:relative' class='rh-icon-img btn-delete'></div>").appendTo(conPwdObj.parent());
			jQuery("#conPswd").css({
				"margin": "5px 5px"
			})
		}
		...
		jQuery("#conPswd").remove();
		jQuery("<div id='conPswd' style='float:left;position:relative' class='rh-icon-img btn-ok'></div>").appendTo(conPwdObj.parent());
		jQuery("#conPswd").css({
			"margin": "5px 5px"
		})
		conFlag=0;
