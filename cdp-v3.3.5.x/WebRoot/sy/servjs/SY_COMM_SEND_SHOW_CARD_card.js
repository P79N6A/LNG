...
btn.bind("click",function(event) {
					...
					var configStr = "SY_COMM_SEND,{'TARGET':'"
							+ inputName
							+ "','SOURCE':'SEND_ID~SEND_NAME~SEND_MEMO','PKHIDE':true,'EXTWHERE':' and 1=1 and S_FLAG = 1 and (S_USER = ^@USER_CODE@^ or S_PUBLIC = 1)','TYPE':'single','ADDBTN':true}";
					var callback = function(value) {
						// 服务ID
						var servId = _viewer.getItem("SERV_ID").getValue();

						// 被分发数据的ID
						var dataId = _viewer.getItem("DATA_ID").getValue();

						// 被分发数据标题
						var dataTitle = _viewer.getItem("DATA_TITLE")
								.getValue();
						// 方案编号
						var sendId = value.SEND_ID;
						// 分发份数
						var sendNum = _viewer.getItem("SEND_NUM").getValue();
						// 分发标示
						var sendFlag = _viewer.getItem("SEND_FLAG").getValue();
						// 分发类型
						var sendType = _viewer.getItem("SEND_TYPE").getValue();
