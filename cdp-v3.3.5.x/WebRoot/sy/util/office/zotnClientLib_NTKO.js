  ...
  zotnClientNTKO.openRedHeadFile = function(downLoadUrl, upLoadUrl, fileName){
	...
	**/
	zotnClientNTKO.DownloadFile = function(DownloadURL, fileName ,ifEncrypt,ifDialog,ifDownload,isRevision){
		...
		if(ifDownload){
			ZotnClient.DownloadFile( DownloadURL, fileName ,ifEncrypt,ifDialog,ifDownload,isRevision);
		}else{
			if(isWordOrExcelFile(fileName)){
...
function showDialog(url,width,height){
	...
	return window.showModalDialog(url,window,"scroll:no;status:off;dialogWidth:" +width+ "px;dialogHeight:"+height+"px");
}