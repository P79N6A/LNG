...
String suggestServer = RhSuggestionClient.getInstance().getServerUri();
String spellcheckServer = RhSpellcheckClient.getInstance().getServerUri();
...
%>
<body dir="ltr" id="gsr" class="" topmargin="3" lang="zh-Hans"
	marginheight="3">

	<div id="main">
		<div>
			<!--rh-头部--搜索和用时开始-->
			<div id="cnt" style="border:0px blue solid;">
			   <table width="100%">
			     <tr class="rh-topTable-tr rh-topTable-trbuttom"><!-- 搜索头第一行开始-->
			         <td class="rh-topTable-tdwid">
			         	<div class="rh-search-icon"></div>
			         </td>
			         <td align=left class="rh-topTable-tdpad">
					      <div id="sfcnt">
							<form id="searchForm"
								style="display: block; background: none repeat scroll 0% 0% transparent; margin: 0pt;"
								action="/SY_PLUG_SEARCH.query.do" id="tsf" method="post"
								onsubmit="Search.search()" role="search">
								<div id="tophf"></div>
								<div>
		
									<div class="tsf-p" style="padding-bottom: 2px;">
										<table
											style="padding: 8px 0pt 0pt; position: relative; z-index: 2;"
											id="sftab" border="0" cellpadding="0" cellspacing="0">
											<tbody>
												<tr>
													<td class="lst-td" width="100%">
														...
														<div class="rh-search-input">
															...
															<div class="rh-search-input-bk">
																<input id="putKeyWords" value='<%
																out.println(keywords);
																%>'
																	title="Ruaho 搜索" autocomplete="off" class="lst"
																	name="titleKeyWordsContent" maxlength="2048" type="text" />
																<input type="hidden" id="data" name="data" value="" /> <input
																	type="hidden" id="lastQuery" name="lastQuery"
																	value="<%=queryStr%>" /> <input type="hidden"
																	id="startTime" name="lastQuery" value="<%=startTime%>" />
																<input type="hidden" id="endTime" name="lastQuery"
																	value="<%=endTime%>" /> <input type="hidden"
																	id="categoryCache" name="categoryCache"
																	value='<%
			                                     			 String selectedCatListStr= JsonUtils.toJson(selectedCatList);
			                                     			  ...
			                                     			  %>' />
																<input type="hidden" id="filterCache" name="filterCache"
																	value='<%
			                                     			 String filterCacheStr= JsonUtils.toJson(filterList);
			                                     			  out.println(filterCacheStr);
			                                     			  %>' />
		
																<span style="display: none;" id="tsf-oq"><%=request.getAttribute("keyWords")%>
																</span>
															</div>
															<div class="rh-search-sea"
																onclick="javascript:Search.search()"></div>
														</div>
													</td>
													<td></td>
													<td>
														<div style="position: relative; height: 30px; z-index: 2;">
															<div class="lsd"></div>
														</div>
													</td>
												</tr>
										...
										<table border="0" cellpadding="0" cellspacing="0">
											<tr>
												<td><div id="suggest"
														style="display: none; left: 18%; top: 57px; width: 609px; z-index: 10"></div></td>
											</tr>
										</table>
									</div>
								</div>
			     ...
			     <tr class="rh-topTable-trbuttom"><!-- 搜索头第二行开始-->
			     	<td>
							<div class="rh-yongshi-img">
								<span class="rh-yongshi-span">智能搜索</span>
							</div>
			         </td>
			         <td  class="rh-topTable-tdpad">
						   <% 
			                	QueryResponse solrResponse = (QueryResponse)outBean.get("SEARCH_RESPONSE");
							 long allResult = pageBean.get(Constant.ALLNUM,0);
							...
							<div id="subform_ctrl" class="rh-yongshi">
								...
								<div class="rh-yongshi-con">
									<div id="resultStats" style="position: relative">
										找到相关结果约<%=allResult%>个
										<nobr> （用时 <%=solrResponse.getQTime()/1000.0%> 秒）</nobr>
			...
			<div class="rh-container" id="nr_container">
				<!--预览开始-->
				<div id="DialogDiv"
					style="display: none; background-color: #FAFAFA; right: 135.5px; top: 100px; position: absolute;">
					<div style="margin-top: 5px">
						<!-- 
				            <div style="height:30px;width:100%;line-height:30px;border-bottom:1px gray solid">
				        -->
						<div>
							<div id="preview-title"
								style="width: 94%; height:100%; padding:10px 0px 0px 10px;float: left; border-bottom:1px #D1E7FF solid;"></div>
							<a href="javascript:Search.close()" id="btnClose"></a>
						</div>
					</div>
					<div id="preview-content"
						style="width: 96%; overflow-y: auto; height: 336px;  padding:0px 2%; font-size:14px; margin: 15px 0; line-height: 23px;"></div>
				</div>
				<!--预览结束-->
									...
									<li class="rh-res-li">
									   ...
									   int docIndex = 0;
									   		for(Bean bean : dataList) {
									   			docIndex++;
									   		String service = bean.getStr(RhSearchClient.getInstance().SOLR_SERVICE);
									   		String abstractText = bean.getStr(RhSearchClient.getInstance().SOLR_ABSTRACT);
									   		...
									   		String title = bean.getStr(RhSearchClient.getInstance().SOLR_TITLE);
									   		String displayTitle = title;
									   		String content = bean.getStr(RhSearchClient.getInstance().SOLR_CONTENT);
									   		String url =  bean.getStr(RhSearchClient.getInstance().SOLR_URL);
									   		String id = bean.getStr(RhSearchClient.getInstance().SOLR_ID);
									   		String contFile = bean.getStr("file_path_strfield");
									   		List<Bean> attList = (List<Bean>)bean.get(RhSearchClient.getInstance().SOLR_ATTACHMENT);
									  		int flag = 1;
												...
												//标题
				  										if (id.startsWith("http://")) {
				  											out.println(" <a target='_blank' href='" +id + "'  class='rh-res-title' >" + displayTitle + "</a> ");
				  										 	...
				  										 	out.println(" <a  href='#' onclick=\"" + url + "\"  class='rh-res-title' >" + displayTitle + "</a> ");
				  										} else {
				  										 	out.println(" <a  href=\"" + url + "\"  class='rh-res-title' >" + displayTitle + "</a> ");
				  										}
											...
											<div style="color: block" class="s">
												<div class="rh-contentPreview">
													<%
										      			 ...
										      			 List<String> relativeList = (List) bean.get(RhSearchClient.getInstance().SOLR_RELATIVE);
										      			 if (relativeList != null && 0 < relativeList.size()) {
										      				 String relativesStr = "";
										      				 for (String relative: relativeList) {
										      					 //not contain self
										      					 if (relative.startsWith(RhIndex.RELATIVE_TYPE.SY_PLUG_SEARCH_SERV.toString())) {
										      			 ...
										      			 title = SearchHelper.keywordsFilter(title);
										      			 ...
										      			 //content-预览
										      				   out.println("<span class=\"rh-res-file-prevSpan\">");
										      		   		   out.println("<a class=\"rh-res-file-prev\" id=\"previews_a" + flag + "\"  href='#' onclick=\"Search.preview(" + flag  + ",'" + id + "','content',this)\"><span class=\"rh-res-file-img\"></span></a>");
										      				   out.println("</span>"); 			 
										      			 %>
												</div>
												<% 
										      			   for (Bean attBean:attList) {
										      				   String attId = attBean.getStr(RhSearchClient.ATTACHMENT_ID);
										      				   String attTitle = attBean.getStr(RhSearchClient.ATTACHMENT_TITLE);
										      				   String attPath = attBean.getStr(RhSearchClient.ATTACHMENT_PATH);
										      				   String attMtype = attBean.getStr(RhSearchClient.ATTACHMENT_MTYPE);
										      				   String attIndex = attBean.getStr(RhSearchClient.ATTACHMENT_INDEX);
										      				   String attContent = attBean.getStr("att_content");
										      				   out.println("<div class=\"rh-res-file-div\">");
															   ...
															   out.println(" <a style='color: #666666;'   href=\"javascript:openOnlinePreviewTab('" + service + "','" + attTitle + "','" + fileUrl +"');\" > 在线预览 </a> ");
										      				   ...
										      				   out.println("<span class=\"rh-res-file-prevSpan\">");
										      		   		   out.println("<a class=\"rh-res-file-prev\" id=\"previews_a" + flag + "\"  href='#' onclick=\"Search.preview(" + flag  + ",'" + id + "','" + previewField +"',this)\"><span class=\"rh-res-file-img\"></span></a>");
										      				   out.println("</span>");
										      				   out.println("</div>");
										      			   }
										      			 %>
										...
										<div class="vsc" sig="mVp">
										  ...
										  %>
											...
											<input type="hidden" id="relevantSearchData"
												...
												value='<%
	                                  	   String relevantSearchStr= JsonUtils.toJson(relevantSearch);
										   relevantSearchStr = SearchHelper.queryEncode(relevantSearchStr);
 ...
 -->
				...
				<div class="tsf-p" id="foot" role="contentinfo">
						...
						<table
							...
							<tbody>
								<tr valign="top">
									<!--分页-->
									<%
					 int pageCurrentPage = pageBean.getInt(Constant.NOWPAGE);
					...
					int pagePageTotal = pageBean.getInt(Constant.PAGES);
					String totalResult = pageBean.getStr(Constant.ALLNUM);
					int startPage = 0;
					int maxPage = 0;
				   ...
				   if(pagePageTotal>pageCurrentPage && !"0".equals(totalResult)){
				  %>
									<!--下一页-->
									<td><a id="pnnext" class="pn"
										style="text-decoration: none; text-align: left;"
