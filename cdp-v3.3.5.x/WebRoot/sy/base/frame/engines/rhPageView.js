...
rh.vi.pageView.prototype._initMainData = function() {
	...
	var setData = FireFly.byId("SY_ORG_USER_STYLE",userCode);
			...
			expandBar.bind("click", function() {
				if(toggled){
					...
					jQuery("#left-homeMenu").animate({ width: GLOBAL.defaultLeftMenuWid}, { duration: 0,complete:function() {
						    ...
						    jQuery("#left-homeMenu").find("h3").unbind('click').bind("mousemove",function(event) {
						       ...
						       if (nextUl.hasClass("leftMenu-ulExtend-show")) {
						       	  ...
						       	  jQuery(".leftMenu-ulExtend-show").hide();
						          nextUl.addClass("leftMenu-ulExtend-show").css({"left":jQuery("#left-homeMenu").width()});
						       	  nextUl.show();
						       }
					...
					jQuery(".ui-tabs-panel").animate({ width: UIConst.PAGE_WID}, { duration: 0,complete:function() {
						    ...
						    jQuery(".leftMenu-ul").width(jQuery("#left-homeMenu").width());
						    jQuery(".leftMenu-ul-show").show();
					}});
...
rh.vi.pageView.prototype._bldBanner = function() {
	...
	var comJsonStr = System.getVar("@C_SY_BANNER_ICON@") || "";
	if (comJsonStr.length > 0) {
		var cmJson = jQuery.parseJSON(comJsonStr);//替换回车
		//"commu":{"url":"SY_COMM_TEMPL.show.do?pkCode=SY_COMMU_CENTER&model=view","name":"通讯中心","menuId":"SY_COMMU_CENTER__@CMPY_CODE@"},
		jQuery.each(cmJson,function(i,n) {
			var title = n.name;
			var icon = i;
			var url = n.url;
			var menuId = Tools.systemVarReplace(n.menuId);
			var temp = jQuery("<div class='rh-head-cm-icon'></div>").attr("title",title).addClass("icon_banner_" + icon).appendTo(cmLinkCon);
			if (i=="portal") {
				temp.bind("click", function(event) {
					var obj = jQuery(".left-topBar-portalArea");
					if (obj.length == 1) {
						if (jQuery(".left-topBar-portalArea:visible").length == 1) {
							jQuery(".left-topBar-portalArea:visible").hide("normal");
						} else {
							jQuery(".left-topBar-portalArea").show("normal");
						}
					} else {
						var area = jQuery("<ul></ul>").addClass("left-topBar-portalArea");
						var data = {"@TDEPT_PT@":{"code":"","name":"@TDEPT_PT@"},//System.getVar("@TDEPT_PT@")
								"@ODEPT_PT@":{"code":"1-QuWdguJ0L8f8tG8YbHMy","name":"公司门户"},//System.getVar("@ODEPT_PT@")
								"@DEPT_PT@":{"code":"1PL1etY999C8jjte_iE68G","name":"部门门户"},//System.getVar("@DEPT_PT@")
								"@USER_PT@":{"code":"29PnOt0Bl8eauAM9M74cVT","name":"个人门户"}};//System.getVar("@USER_PT@")
						jQuery.each(data, function(i,n) {
							var ptCode = n.code;
							var name = n.name;
							if ((ptCode.length ==0) && (i != "@USER_PT@")) {//非个人门户的主键为空时返回
								return;
							}
							var content = jQuery("<a href='#' value='self'>" + name + "</a>");
							content.on("click",function(event) {//显示列表的点击
								var val = i;
								if (val == "@USER_PT@") {
									var options = {"url":"SY_COMM_TEMPL.show.do?model=view&action=new&type=5&pkCode=" + ptCode,
											"menuFlag":3,"menuId":"SY_TIME_CENTER__" + System.getVar("@CMPY_CODE@"),"tTitle":jQuery(this).text()};
									Tab.open(options);	
								} else {
									var options = {"url":"SY_COMM_TEMPL.show.do?model=view&pkCode=" + ptCode,
											"menuFlag":3,"menuId":"SY_TIME_CENTER__" + System.getVar("@CMPY_CODE@"),"tTitle":jQuery(this).text()};
									Tab.open(options);				
								}
								event.stopPropagation();
							});
							var li = jQuery("<li></li>").append(content);
							if (i == "@USER_PT@") {//个人门户的编辑按钮
								var span = jQuery("<span title='" + name + "编辑' style='cursor:pointer;padding:2px 8px 0px 8px;font-size:12px;margin-left:5px;' class='btn-edit'></span>");
								span.bind("click",function(event) {
									var options = {"url":"SY_COMM_TEMPL.show.do?model=edit&action=new&type=5&pkCode=" + ptCode,//model=edit&action=new&type=5
											"menuFlag":3,"menuId":"SY_TIME_CENTER__" + System.getVar("@CMPY_CODE@"),"tTitle":name + "编辑"};
									Tab.open(options);
									event.stopPropagation();
								});
								li.append(span);
				...
				temp.bind("click", function(event) {
					var options = {"url":url,"menuFlag":1,"menuId":menuId,"tTitle":title};
					Tab.open(options);
				});
			} 
		});
	...
	if (typeof bannerSearch == "string" && (bannerSearch !== "false")) {
		...
		var temp = jQuery("<input type='text' class='rh-head-sea-input boxShadowInTop3 radius5' value='全文搜索'/>").appendTo(seaCon);
		...
		seaIcon.bind("click",function(event) {
			...
			if ((jQuery.trim(keywords).length > 0) && (keywords != "全文搜索")) {
				var keyStr = encodeURIComponent(keywords);
				var opts = {"sId":"SEARCH-RES","tTitle":"搜索","url":"/SY_PLUG_SEARCH.query.do?data={'KEYWORDS':'" + keyStr + "'}","menuFlag":3};
				Tab.open(opts);
			}
		});
	...
	if (typeof comJsonStr == "string" && comJsonStr.length == 0) {//无banner图标则靠右
		perCon.addClass("rh-head-per-right");
		jianListAbsoluteClass = "rh-head-per-right";
	} else {
		perCon.addClass("rh-head-per-left");
	}
	var perConIn = jQuery("<div></div>").addClass("rh-head-per-in").appendTo(perCon);
	var userSex = System.getVar("@USER_SEX@");
	var perImg = "";
	if (userSex== "0") {
		perImg = FireFly.getContextPath() + "/sy/theme/default/images/common/male-icon-32px.png";
	} else if (userSex == "1") {
		perImg = FireFly.getContextPath() + "/sy/theme/default/images/common/girl-icon-32px.png";
	}
	if (System.getUser("USER_IMG") && (System.getUser("USER_IMG").length > 0)) {
		var img = System.getUser("USER_IMG").split(",")[0];
		perImg = FireFly.getContextPath() + "/file/" + img + "&size=100x100";
	}
	var perDiv = jQuery("<img class='rh-head-per-icon cp' title='进入个人基本信息'></img>").attr("src",perImg).appendTo(perConIn);
	...
	perDiv.bind("click", function(event) {
		var options = {"url":"SY_ORG_USER_CENTER.show.do","menuFlag":2
				,"menuId":"SY_USER_INFO__" + System.getVar("@CMPY_CODE@"),"tTitle":"基本信息"};
		Tab.open(options);
	})
	//委托
	if (false) {
	...
	if (System.getVar("@JIAN_CODES@").length > 0) {
		...
		jianGang.bind("click", function() {
			...
			if (obj.length == 1) {
				...
				var data = FireFly.doAct("SY_ORG_LOGIN","getJianUsers",null,false);
				jQuery.each(data._DATA_,function(i,n) {
					var content = jQuery("<a href='#' class='left-top-jian-list--li--a'>" + n.USER_NAME + "  (<span style='color:red;'>" + n.TODO_COUNT + "条</span>)" + "</a>");
					content.bind("click",function(event) {
						var result = FireFly.doAct("SY_ORG_LOGIN","changeUser",{"TO_USER_CODE":n.USER_CODE},false);
						if (result[UIConst.RTN_MSG].indexOf(UIConst.RTN_OK) == 0) {
							var res = confirm("当前页面将刷新，确定继续吗？");
					...
					aCon.append(content);
					li.append(aCon)
					li.append("<img title='' class='left-topBar-jian-list--img' src='/sy/theme/default/images/common/girl-icon-32px.png'/>");
...
rh.vi.pageView.prototype._bldTabs = function() {
	...
	jQuery("#loginOut").on("click", function(event) {//退出
	       ...
	       if (resultData[UIConst.RTN_MSG].indexOf(UIConst.RTN_OK) == 0) {
		       var homeUrl = FireFly.getContextPath() + "/";
		        //若开启单点登录需要使用下句
		       //var homeUrl = FireFly.getContextPath() + "/logout.jsp";
	...
	jQuery(".rh-head-close").bind("click", function(event) {//展开收起banner方法
	   ...
	   if (expand.hasClass("rh-head-expand")) {
	       ...
	       if (platformPage.length > 0) {
		       document.getElementById("platformFrame").style.height = jQuery("#platformFrame").height() - 45 + "px";
		   }
	...
	if (this.homeOpts && this.homeOpts != "null" && (platformPage.length == 0)) {//打开默认home
		...
		var platformFrame = jQuery("<iframe id='platformFrame' name='platformFrame' src='"
				+ FireFly.contextPath + "/sy/comm/desk/desk.jsp' border='0' scrolling=no width=100% height=100% frameborder='0'></iframe>").addClass("tabFrame").appendTo(platformPage);
		//信息平台Tab初始化
...
rh.vi.pageView.prototype._preBldTopPannel = function(){
	...
	var opnerSpan = jQuery("<span>&nbsp;</span>").attr("id","rh-slideMsgBtn").addClass("rh-slideMsgBtn").appendTo(opner);
    _self.div = null;
	_self.title = null;
	_self.icon = null;
	_self.tit = null;
...
rh.vi.pageView.prototype._bldBoxHtml = function() {
	...
	appboxHtml +=         '<span id="btnAllSet" class="rhDesk-btn icon-dialog-all">整体样式</span>';
...
//	appboxHtml +=         '<span class="rhDesk-btn-line"></span>';
	appboxHtml +=         '<span id="btnDefaultSet" class="rhDesk-btn" style="text-decoration: underline;">恢复风格</span>';
	appboxHtml +=      '</div>';
...
rh.vi.pageView.prototype._afterLoad = function() {
	...
	var userCode = System.getUser("USER_CODE");
	setTimeout(function() {
		if (agtFlag == UIConst.STR_YES) {//代理的判断
			var flag = confirm("您的账号处于委托代理状态，是否取消工作委托？");
			if (flag == true) {
				//var res = FireFly.doAct("SY_ORG_USER_AGENT","endAgent",{'USER_CODE':userCode},true);
				var param = {"_PK_":"","AGT_STATUS": 0,"action":"stopAgent"};
				var res = FireFly.doAct("SY_ORG_USER_TYPE_AGENT", "doStopAgent", param, true);
			}
		}
	},1000);
	//4.个人资料完整的的比例获取
	...
	if (percent.deg) {
		jQuery("#rh-head-per--rate").text(percent.deg + "%");
	}
	//5.提示处理
