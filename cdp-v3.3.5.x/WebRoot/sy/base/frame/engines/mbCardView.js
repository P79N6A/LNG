...
mb.vi.cardView.prototype._afterLoad = function() {
	...
	if (this.topTitle == null) {
		this.servName = this._data.SERV_NAME;
		this.topTitle = jQuery("<div></div>").text(this.servName).addClass("mbTopBar-title").appendTo(this.top.find(".mbTopBar-center"));	
	}
...
mb.vi.cardView.prototype.cardBarTip = function(msg) {
	Tip.show(msg,null,this.getNowDom());
};
/*
 * 错误的提示信息
 * @param msg 提示内容
...
mb.vi.cardView.prototype.cardBarTipError = function(msg) {
	Tip.showError(msg,null,this.getNowDom());
};
/*
 * 清除加载提示信息
 */
