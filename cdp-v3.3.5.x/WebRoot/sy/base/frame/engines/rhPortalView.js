...
rh.vi.portalView = function(options) {
	...
	this._type = this.opts.type || "";  //添加的类型，5:个人
...
rh.vi.portalView.prototype._editModel = function() {
	...
	var theme = jQuery("<select style='border:1px gray solid;margin-left:20px;' title='选择模版的风格，点击保存生效'><option value='portal-null'>无</option><option value='portal-blue'>蓝</option><option value='portal-orange'>桔色</option><option value='portal-gray'>灰</option></select>")
...
rh.vi.portalView.prototype._openComsView = function(pcType) {
	...
	if (this._type.length > 0) {
		_self._bldComsContent(null,this._type);
	} else {
		//构造标题分类
		...
		jQuery.each(data, function(i,n) {
			var item = jQuery("<span class='ui-dialog-title cp'>" + n.NAME + "</span>").appendTo(titBar);
			item.bind("click", function (event) {
				jQuery(".portal-com-dialog-titleActive").removeClass("portal-com-dialog-titleActive");
				jQuery(this).addClass("portal-com-dialog-titleActive");
				_self._bldComsContent(n.ID);
			});
		});
		_self._bldComsContent();		
	}
...
rh.vi.portalView.prototype._bldComsContent = function(pcType,areaType) {
	var _self = this;
	parent.Tip.showLoad("加载中");
	var pcTypeParam = pcType || "";
	var areaTypeParam = areaType || "";
	this.comIframe.attr("src",FireFly.getContextPath() + "/sy/comm/home/portalAllComs.jsp?pcType=" + pcTypeParam + "&areaType=" + areaTypeParam);
};
...
rh.vi.portalView.prototype._bldParamContent = function(paramObj) {
	...
	jQuery("<div style='color:red;'>&nbsp;&nbsp;(注：输入项为空则使用默认值,设定完成后点击顶部保存生效！)</div>").addClass("portal-param-edit-line").appendTo(comsContainer);
	var i = 0;
	var len = data.length;
    ...
    jQuery.each(data,function(i,n) {//构造选择组件页面的每一个组件
        ...
        jQuery("<label></label>").addClass("portal-param-edit-label").text(name).appendTo(div);
        var input = jQuery(); 
        if ((id == "data")) {//设置数据对象
        	if (pcType < 20) {
        		return;
        	}
        	input = jQuery("<textarea id='" + id + "' rows=3 cols=30></textarea>").addClass("portal-param-edit-input").appendTo(div);
        	var addBtn = jQuery("<span class='cp ul' style='color:blue;' title='栏目选择'>选择栏目</span>").appendTo(div);
        		...
        		if (pcFtl == "SY_COM_BANNER.ftl") {//新闻banner型
            		...
            		var options = {
            			...
            			"replaceCallBack":function(idArray,nameArray) {
            				//构造选择栏目的数据对象字符串
            				var dataurl = "SY_COMM_CMS_CHNL.query.do?data={\"_extWhere\":\" and CHNL_ID in ('" + idArray.join("','") + "')\"}";
            				input.val(dataurl);
            			}
            		};
        		} else {
        	...
        	input = jQuery("<select id='" + id + "' class='portal-param-edit-select'><option value='portal-null'>无</option><option value=''>标题条式</option><option value='portal-box-border'>边框式</option></select>").appendTo(div);
            input.addClass("portal-param-edit-input");
        } else if (id == "titleBar") {//标题条
        	input = jQuery("<select id='" + id + "' class='portal-param-edit-select'><option value='portal-null'>无</option><option value=''>显示</option><option value='portal-title-none'>不显示</option></select>").appendTo(div);
            input.addClass("portal-param-edit-input");
        } else {
        	div.addClass("portal-param-edit-line");
        	input = jQuery("<input id='" + id + "'></input>").addClass("portal-param-edit-input").appendTo(div);
        }

        if (id === "height") {//高度
        	var addBtn = jQuery("<span class='portal-param-edit-add' title='增加高度'> + </span>").appendTo(div);
        	addBtn.bind("click", function() {
        	...
        	var deleteBtn = jQuery("<span class='portal-param-edit-add' title='减少高度'> — </span>").appendTo(div);
        	...
        	var span = jQuery("<span class='cp ul' style='color:blue;'>选择图标</span>").appendTo(div);
        	span.bind("click",function(event) {
        		if (jQuery(".portal-param-icons").length == 1) {
        			jQuery(".portal-param-icons").show();
        		} else {
        			var icons = jQuery("<div class='portal-param-icons'></div>").appendTo(div);
    	...
    	var clearBtn = jQuery("<span class='cp ul portal-param-edit-clear' title='清空'>清空</span>").appendTo(div);
    	clearBtn.bind("click", function() {
    		input.val("");
    	});
        div.appendTo(comsContainer);
	});
	//2.=========系统参数：获取组件参数定义数据==============
    ...
    jQuery.each(pcSelfParam, function(i,n) {
        ...
        jQuery("<label></label>").attr("title",n.name).addClass("portal-param-edit-label").text(name).appendTo(div);
        div.addClass("portal-param-edit-line");
    	if (type && type == 2) {//组件参数设定需要显示并后台需要替换的
    		input = jQuery("<input id=''></input>").attr("repId",id).addClass("portal-param-edit-input").appendTo(div);	
    	} else {
        	input = jQuery("<input></input>").attr("id",id).addClass("portal-param-edit-input").appendTo(div);	
    	}
    	if (paramObj[id]) {
    		input.val(paramObj[id]);
    	}	
	...
	var div = jQuery("<div style='margin-top:20px;'></div>").addClass("portal-param-edit-line")
	var okParam = jQuery("<a class='rh-icon rhGrid-btnBar-a portal-param-edit-ok' style='margin-left:36%'><span class='rh-icon-inner'>确定</span><span class='rh-icon-img btn-ok'></span></a>").appendTo(div);
	...
	var closeParam = jQuery("<a class='rh-icon rhGrid-btnBar-a portal-param-edit-ok' style='display:block-inline;'><span class='rh-icon-inner'>关闭</span><span class='rh-icon-img btn-delete'></span></a>").appendTo(div);
...
rh.vi.portalView.prototype._saveTempl = function() {
    ...
    jQuery(".portal-target",hml).each(function(i,n) {//取body内的portal-target对象，过滤掉其它的杂项
    	if (jQuery(n).parent().get(0).tagName == "TD") {
    		jQuery(n).parent().parent().parent().appendTo(ptContent);
    	} else {
    ...
    if (this._action == "new") {//新建的模版
    	data["PT_TITLE"] = this._paramBean["PT_TITLE"] + "[" + System.getVar("@USER_CODE@") + "]";
    	data["PT_TYPE"] = this._type;
    	data["PT_TYPE_DEFAULT"] = "2";
    }
 ...
 * 设置页面高度
 */
rh.vi.portalView.prototype._setPortalHei = function() {
