...
rh.vi.cardView = function(options) {
   ...
   this._title = (this.opts.title.length > 0) ? this.opts.title : this.servName;
 ...
 * 显示卡片页面，主方法
 */
rh.vi.cardView.prototype.show = function() {
	var _self = this;
...
rh.vi.cardView.prototype._tabLayout = function() {
   ...
   //tempTODO 临时增加边框阴影
   this.winDialog = jQuery("<div></div>").addClass("cardDialog").addClass("rh-bottom-right-radius").attr("id",this.dialogId).attr("title","卡片").css(winDialogCss);
   ...
   if (parent.GLOBAL.style.SS_STYLE_BLOCK) {//区块头
	   this.mainUL.addClass(parent.GLOBAL.style.SS_STYLE_BLOCK);
   }
   var mainLi = jQuery("<li></li>").addClass("rhCard-tabs-li").appendTo(this.mainUL);
   var mainA = jQuery("<a></a>").text("主信息").attr("href","#"+ this._mainTab).appendTo(mainLi);
   this.mainLi = mainA;
   mainA.bind("click",function() {
   	   _self._changeTabHeiWid(_self._MainHeight);
   });
   this.mainCon = jQuery("<div></div>").attr("id",this._mainTab).appendTo(this.tabs);
   ...
   if (this._actVar == UIConst.ACT_CARD_MODIFY) {
	   jQuery.each(this._linkServ,function(i,n) {
		  ...
		  if ((n.LINK_SHOW_POSITION == 2) || n.LINK_SHOW_POSITION == 3) {
		   	  var tempA = jQuery("<a></a>").attr("href","#" + n.LINK_SERV_ID).append(n.LINK_NAME);
		   		  ...
		   		  setTimeout(function() {//第一次加载
			   	  	  ...
			   	  	  jQuery.each(linkItem, function(i,n) {//生成子功能过滤条件
				   	  	...
				   	  	if (n.LINK_VALUE_FLAG == 2) {//主单常量值
			   	  	  	    ...
			   	  	  	    } else if (n.ITEM_CODE.toUpperCase() == "TYPE") { //单选多选标识
			   	  	  	        linkItemType = n.LINK_ITEM_CODE;
			   	  	  	    } else {//其它常量值的增加
			   	  	  ...
			   	  	  if (n.LINK_SHOW_TYPE == 3) {//自定义url
					   	  ...
					   	  var temp = {"sId":n.LINK_SERV_ID,"pCon":tempCon,"showSearchFlag":"false","showTitleBarFlag":"false","replaceQueryModel":1,"type":linkItemType,
   ...
   if (this.opts.backBtn === true) {//显示返回按钮
	   var backLi = jQuery("<li></li>").addClass("rhCard-backLi").appendTo(_self.mainUL);
	   this.backA = jQuery("<a>返回</a>").attr("id","rhCard-back").addClass("rhCard-refresh").appendTo(backLi);
...
rh.vi.cardView.prototype._bldURLTab = function(opts) {
   ...
   var frame = jQuery("<iframe border='0' frameborder='0'></iframe>").attr({"id":id,"name":id,"width":"100%","height":600,"src":tabUrl});
...
rh.vi.cardView.prototype._bldWin = function() {
    ...
    var widPercent = "98.1%";
    var parWid = parent.document.documentElement.scrollWidth;
    ...
    jQuery("#" + this.tabsId).tabs({
//        select: function(event, ui) { //响应点击Tab事件
//        	var delayFlag = "_delay_display_tab"; //延时显示标记
//        	var uiTabObj = jQuery(ui.tab);
//        	if(uiTabObj.hasClass(delayFlag)){ //是否有延时显示标记，如果有则显示
//        		uiTabObj.removeClass(delayFlag);
//        	}else{
//        		var hasHidden = _self.hideControlObj(); // 隐藏Object对象
//        		if(hasHidden){ //如果有需要隐藏的Object，则延时进入选中的Tab
//    	        	window.setTimeout(function(){ //延时50毫秒，显示Tab
//    	        		jQuery("#" + _self.tabsId).tabs('select', ui.index);
//    	        	},50);
//    	        	uiTabObj.addClass(delayFlag); //增加延时显示标记
//    	        	return false;        			
//        		}
//        	}
//        	
//        	_self.displayControlObj(jQuery(ui.panel)); //显示控件Object
//        	return true;
//        }
    });
    ...
    if (this.miniCard) {//有替换的宽高度
    	this.winDialog.addClass("rh-ui-dialog-border");
    }
...
rh.vi.cardView.prototype._readOnlyForm = function() {
    ...
    if (_self._readOnly == true || _self._readOnly == "true"　|| editBtnsFlag == false) {
		...
		// 设置意见只读
//		if (_self.wfCard) {
//			_self.wfCard.mind.setMindDisabed();
//		}
    }
};
/*
 ...
 * 构建卡片按钮条-卡片下方
 */
rh.vi.cardView.prototype._bldBtnBarBottom = function() {
    var _self = this;
	var btns =  this.btnBar.find(".rhGrid-btnBar-a:visible");
	var btnBar = jQuery("<div></div>").addClass("rhCard-btnBar-bottom");
	jQuery.each(btns,function(i,n) {
		var a = jQuery(n).clone();
		a.on("click",function(event) {
			var code = a.attr("actcode");
			if (jQuery(this).hasClass("rhCard-btnBar-wfNext")) {//下一步
				_self.wfCard._getWfNextBtn(code).layoutObj.click();
			} else if (jQuery(this).hasClass("rhCard-btnBar-wf")) {//正常
				_self.wfCard._getBtn(code).layoutObj.click();
			} else {
				_self.getBtn(code).click();
			}
		});
		a.appendTo(btnBar);
	});
	btnBar.insertAfter(this.formCon);
};
/*
 * 构建卡片按钮条后执行
 */
...
rh.vi.cardView.prototype._judgeEditBtns = function() {
	  ...
	  if (_self._judgeEditBtnsVar) {
		  ...
		  _self._firstJudage = rtnFlag;//记录上一次正确的
	  }
	  if (_self._firstJudage) {
		  rtnFlag = _self._firstJudage;
	  }
      return rtnFlag;
};
/*
...
rh.vi.cardView.prototype._bldForm = function() {
    var _self = this;
    //构造Form
    this.formCon = jQuery("<div class='form-container'></div>");
	var opts = {
		"id":"formView",
		"pId":_self.opts.sId,
		 cols : 3,
...
rh.vi.cardView.prototype._fillFormData = function() {
    ...
    if (this._actVar == UIConst.ACT_CARD_MODIFY) {//修改
 	   var data = this._beforeByIdData || this._byIdData();
...
rh.vi.cardView.prototype._saveForm = function() {
    ...
    if (this._actVar == UIConst.ACT_CARD_MODIFY) {//修改
        ...
        _self.addSave(changeData);
    }
    return true;
};
/*
 * 添加保存
 * @param afterReload 保存后是否刷新页面 默认true
 */
 ...
 * @param afterReload 保存后是否刷新页面 默认true
 */
rh.vi.cardView.prototype.modifySave = function(changeData,afterReload) {
	var _self = this;
    changeData[UIConst.PK_KEY] = this._pkCode;	
 ...
 * 显示评论列表
 */
rh.vi.cardView.prototype.showComments = function(NOWPAGE) {
	var _self = this;
	jQuery("#commentDiv"+_self.opts.sId).remove();

	var reqComment = {};
	reqComment.SERV_ID = _self.opts.sId;
	reqComment.DATA_ID = _self._pkCode;
	reqComment.NOWPAGE = NOWPAGE;
    var resultData = FireFly.doAct("SY_SERV_COMMENT","getCommentList",reqComment);
    var commentDiv = jQuery("<div id='commentDiv"+_self.opts.sId+"' style='margin-left:60px'></div>");
    var commentFormCon = jQuery("#commentFormConent"+ _self.opts.sId);
    commentDiv.appendTo(commentFormCon);
    //显示输入框
	var commentInput = jQuery("<div><textarea name='commentInput"+_self.opts.sId+"' id='commentInput"+_self.opts.sId+"' cols='90' rows='4' value=''></textarea></div>");
	commentInput.appendTo(commentDiv);
	
	var deploybtn = jQuery("<a href='#' id='deployComment"+_self.opts.sId+"'>发布</a>")
	deploybtn.appendTo(commentInput);
	deploybtn.bind("click",function() {
	    var commentVal = jQuery("#commentInput" + _self.opts.sId).val();
	    _self.addComment(commentVal);
	});    
    //显示列表
    jQuery.each(resultData._DATA_,function(i,commentItem) {
    	jQuery("<span style='line-height: 200%'>"+commentItem.S_USER__NAME+"&nbsp;&nbsp;"+commentItem.S_CTIME+"</span><br>").appendTo(commentDiv);
    	jQuery("<span>"+commentItem.C_CONTENT+"</span><br>").appendTo(commentDiv);
    });
    //显示分页
    var ALLNUM = resultData._PAGE_.ALLNUM;
    var SHOWNUM = resultData._PAGE_.SHOWNUM;
    var PAGES = resultData._PAGE_.PAGES;
    var NOWPAGE = resultData._PAGE_.NOWPAGE;
    function Page (i) {
    	this.goPage = function() {
    		_self.showComments(i);
    	};
    }
    if (PAGES > 1) { //页数大于1
    	for (var i=1;i<=PAGES;i++) {
    		if (NOWPAGE == i) {
    			jQuery("<span>"+i+"&nbsp;&nbsp;&nbsp;&nbsp;</span>").appendTo(commentDiv);
    		} else {
    			var commentLink = jQuery("<span><a href='#' id='commentPage"+i+"'>"+i+"</a>&nbsp;&nbsp;&nbsp;&nbsp;</span>");
    			commentLink.appendTo(commentDiv);
    			var func = new Page(i);
    			commentLink.unbind("click").bind("click",func.goPage);
    		}
    	}
    }
};
 ...
 * 添加评论
 */
rh.vi.cardView.prototype.addComment = function(commentVal) {
	var _self = this;
	var commentObj = {};
	commentObj.SERV_ID = _self.opts.sId;
	commentObj.DATA_ID = _self._pkCode;
	commentObj.C_CONTENT = commentVal;
	commentObj.S_CTIME = rhDate.getTime();
	var resultData = FireFly.doAct("SY_SERV_COMMENT", "save" ,commentObj);
	//刷新评论列表 
	_self.showComments(1);
	this._resetHeiWid();
};
...
rh.vi.cardView.prototype._resetHeiWid = function(bottomHei) {
    ...
    jQuery(document).ready(function() {
    	...
    	if (tempH > 0) {//正常显示时预存高度，解决从另外tab刷新当前卡片时取不到高度问题
    		...
    		tempH = _self.tabs.data("visibleHeight");
    	}  	
    	if (bottomHei) {//有高度差时
    		var extraHeight = 35 + 35 + _self.headMsgBar.prop("offsetHeight") + 15 + 35;  //tab栏+上工具栏+提示信息栏+form的margin+下工具栏
    		tempH = _self.formCon.height() + bottomHei + extraHeight; //form+按钮条+标签条
    	}
    	if ((_self._cardIn == true) && (_self.opts.bottomTabFlag == true)) {
    		_self._height = _self.tabs.height() + 35;//单条记录进卡片时预存到tab点击时缓存高度
    		_self.opts.parHandler._resetHeiWid(_self._height);
    		return false;
    	}
    	//关联card在右侧显示
...
rh.vi.cardView.prototype._tipShow = function() {
    if (this.tempTip) {
    	this.tempTip.appendTo(jQuery(".tabUL").last());      	
    } else {
    	Tip.clearLoad();
    }
};
...
rh.vi.cardView.prototype._excuteProjectJS = function() {
	var _self = this;
	var jsLoadFlag = this._servCardLoad;
	var pLoadFlag = this._pServCardLoad;
	var pSId = this._servPId;//父服务
	var sId = this.opts.sId;//当前服务
	if ((this._servPId.length > 0) && (pLoadFlag == 1)) {//有当前服务的父卡片js
	    load(pSId);	
	}
	if (jsLoadFlag == 1) {//有当前服务卡片js
        load(sId);
	}
	function load(value) {
...
rh.vi.cardView.prototype.slimParams = function() {
	...
	if (this.params) {
		jQuery.each(this.params,function(i,n) {
			if (i == "handler" || i == "viewer" || i == "callBackHandler" || i == "closeCallBackFunc" || i.indexOf("Handler") > 0) {
			} else {
				params[i] = n;
			}
		});
