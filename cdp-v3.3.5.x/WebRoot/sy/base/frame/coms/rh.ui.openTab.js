...
var Tab = {
	...
	open: function (options) {
		...
		if (System.getMB("flag") == true) {
			...
			if (act == "list") {
				...
				var tab = new parent.rh.ui.openTab(options);
				tab.render();
			} else if (act == "card") {
				var repUrl = url;
	...
	setCardTabFrameHei : function () {
		var allHei = document.documentElement.scrollHeight;
		_parentViewer.winDialog.height(allHei+40);
		Tab.setFrameHei(allHei+40);
		parent.Tab.setFrameHei(allHei+50);
		var sid = window.name.split("-tabsIframe")[0];//取得当前关联服务code
		_parentViewer.sonTab[sid]._height = allHei+40;
	},
	/**
	 * 首页tab条的浮动方法
	 */
...
rh.ui.openTab = function(options) {
	...
	if (this.act == "card") {//打开卡片的默认url
		...
		+ "&paramsFlag=" + this.paramsFlag + "&pkCode=" + this.pkCode + "&readOnly=" + this.readOnly	
		+ "&areaId=" + this.areaId + "&title=" + encodeURI(this.tTitle);
		...
		if (urlArray.length > 1) {
			...
			jQuery.each(last,function(i,n) {
				...
				if (temp[0] == "readOnly") {
					_self.readOnly = temp[1];
				} else if (temp[0] == "extWhere") {
					_self.extWhere = temp[1];
				} else if (temp[0] == "dataFlag") {
					_self.dataFlag = temp[1];
				} else if (temp[0] == "parWhere") {
					_self.parWhere = temp[1];
				} else if (temp[0] == "parVar") {
					_self.parVar = temp[1];
				} else if (temp[0] == "type") {
					_self.type = temp[1];
				}
		...
		if (this.extWhereRep) {//用另外的参数
			_self.extWhere = this.extWhereRep;
		}
		this.url = FireFly.contextPath + "/sy/base/view/stdListView.jsp?frameId=" + this.frameId + "&sId=" +　this.sId 
		            + "&paramsFlag=" + this.paramsFlag + "&readOnly=" + this.readOnly + "&extWhere=" + this.extWhere + "&title=" + encodeURI(this.tTitle)
		            + "&dataFlag=" + this.dataFlag + "&parWhere=" + _self.parWhere + "&parVar=" + _self.parVar + "&type=" + _self.type;	
...
rh.ui.openTab.prototype._bldTabFirst = function() {
	 ...
	 //构造tab和iframe
	 var tempDiv = jQuery("<div style='float:right;'></div>").height(GLOBAL.getDefaultFrameHei()).width(UIConst.PAGE_WID);
	 ...
	 if (this.act == "list") {//打开系统支持页面
	 	 Tip.showLoad("加载中...");
	 	 this.frameObj.attr("src",this.url);
...
rh.ui.openTab.prototype._refreshTab = function() {
 	 jQuery("a[href='#" + this.parDiv +　"']").data("iframeObj").attr("src",this.url);;
};
/*
 * 组件执行后方法
 */
    	 ...
    	 if (jQuery(".rh_tabs_more").length == 0) {
    	     ...
    	     moreCount.bind("click",function() {
    	    	 ...
    	    	 if (moreDiv.length == 1) {
    	    		 ...
    	    		 jQuery(".rh_tabs_more").css("right",10).css("top",35).show();
    	    	 }
    	     });
    	 }
    	 //最后一个移动至更多区域
...
rh.ui.openTab.prototype._resetMoreCount = function() {
	if (jQuery(".rh_tabs_more .rh_tabs_more_a").length > 0) {
		jQuery(".rh_tabs_moreCount").html("<sup>>></sup>" + jQuery(".rh_tabs_more .rh_tabs_more_a").length);	
