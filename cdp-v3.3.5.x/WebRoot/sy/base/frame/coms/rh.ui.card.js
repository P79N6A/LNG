...
rh.ui.Label = function(options) {
	...
	// 文本对象
	this._text = jQuery("<span class='name' style='cursor:pointer;'>" + opts.text + "</span>");
	// 星号
			...
			if (!_self.isNull()) {
				if (opts.fieldType == UIConst.DATA_TYPE_NUM && pass) {// 数字校验
					...
					if (!jQuery(this).validate(
							...
							"长度不能超过" + opts.length + "位！")) {
						pass = false;
					}
				}

				if (opts.regular && pass) {// 正则校验
					jQuery(this).validate(opts.regular, _self.getValue(),
							opts.hint);
				}
			}
		});
...
rh.ui.Checkbox.prototype.show = function() {
	this.isHidden = false;
	this._container.parent().parent().parent().show();
};
/**
 * 设置Checkbox为必须输入
 */
...
rh.ui.Radio = function(options) {
	...
	if (this._tip.length > 0) {
		...
		jQuery("<span></span>").addClass("ui-label-after").html(this._tip).appendTo(this._container.find("#" + opts.id).first());
	}
};
/**
 * 获取blank
...
rh.ui.Radio.prototype.getLabel = function() {
	return this._obj.parent().parent().find("#" + this._opts.id + "_label");
};
/**
 * 获取容器
 */
 ...
 */
rh.ui.Select.prototype.fillData = function(val) {
	var rtnVal = false;
	if (val) {
...
rh.ui.Select.prototype.addOptions = function(data) {
	...
	if(jQuery.isEmptyObject(data)){  //保证必须有一个空的option，否则ie9下显示不正确
		this._obj.append("<option value='' selected></option>");
		return;
	}
	var len = data.length;
	if (this._obj.find("option").length == 0) {  // 避免从外部调用addOptions时出现两个空白option
		this._obj.append("<option value='' selected></option>");
	}
	for (var i = 0; i < len; i++) {
		var value = data[i].ID;
 ...
 */
rh.ui.Select.prototype.setValue = function(val) {
	return this.fillData(val);
};
/**
 * 获取选中的option的文本
 */
...
rh.ui.File = function(options) {
	...
	$strArr.push("<table style='border-width:0'><tr style='border-width:0' class='uploadBtntr'><td style='border-width:0'>");
    
	
	// 按钮
	$strArr.push("<span class='uploadBtn'><a class='rh-icon rhGrid-btnBar-a' id='" + this.time  + "-upload' actcode='upload'>" +
			"<span class='rh-icon-inner'><span id='uploadHolder_" + this.time + "'></span></span></a><span class='modifyBtn'><a class='rh-icon rhGrid-btnBar-a' actcode='upload'>" +
	...
	this._container = this.obj = jQuery("<div class='blank' style='border-width:0 0;background-image:none;'></div>").append(this._obj);
	
	// 设置宽度
	if (opts.width && parseInt(opts.width) > 0) {
		this._container.width(opts.width);
	}
	// 默认文件权限为15，除修改名字之外的全部权限
	this._acl = 15;
	...
	var modfiyAcl = aclArr[0];
	if (!modfiyAcl) {
		modify.hide();
	}
	
	// 自定义事件
	this.beforeUploadCallback = function() {return true;};// 上传之前
...
rh.ui.File.prototype.validate = function() {
	if(this._opts.isNotNull){
		if(this.isNull()){
			this._container.showError("必须上传文件");
		}else{
...
/**
 * 设置文件读写权限
 * 设置文件的权限，与二进制11111进行与运算，最后转换成一个布尔类型的数组，把“修改，删除，上传，编辑，查看”
 * 1可读 10可编辑 100可上传 1000可删除 10000可修改
 * @param acl权限数值
 */
...
rh.ui.File.prototype.destroy = function() {
	if (this._upload) {
		var swfObj = this._upload.getSwfObj();
		swfObj && swfObj.destroy();
	}
};
/**
 * 清空
...
rh.ui.File.prototype.initUpload = function() {
	...
	if (!this._upload) {
		...
		// 上传之前做处理
		this._upload.beforeUpload = function() {
			
			// 显示进度条
			_self.showProgress();
			
			// 调用对外回调函数
			if (!_self.beforeUploadCallback.call(_self)) {
 ...
 * 显示进度条
 */
rh.ui.File.prototype.showProgress = function() {
	var hei = 300;
	var wid = 450;
...
rh.ui.File.prototype.fillData = function(fileData){	
	...
	for (var i = 0; i < len; i++) {
		...
		$strArr.push("<span class='fileName'>");
		$strArr.push("<a title='" + name + "' href='" +  FireFly.getContextPath() + "/file/" + fileId + "?act=download' target='_blank'>");
		$strArr.push(disName);
		$strArr.push("</a>");
		$strArr.push("</span>");
		...
		// 注册编辑事件
		$edit.data("id", fileId).data("name", name).click(function(){
			var $this = jQuery(this);
			_self.editFile($this.data("id"), $this.data("name"));
		});
...
rh.ui.File.prototype.modifyFile = function() {
	...
	if (idArr.length > 0) {
	    ...
	    var temp = {"sId":"SY_MODIFY_FILE","pCon":div,"batchFlag":true,"showTitleBarFlag":"false",
 ...
 * @param fileId 文件ID
 */
rh.ui.File.prototype.editFile = function(fileId, fileName) {
	var uploadUrl = "/file/" + fileId;
	if (this._saveHist) {
		uploadUrl = uploadUrl + "?model=saveHist";
	}
	editOfficeFile(fileName, "/file/" + fileId, uploadUrl, true);
};
/**
 * 查看文件
 * @param fileId 文件ID
...
rh.ui.File.prototype.calcAcl = function(acl) {
	var totalLen = 5; // 权限2进制总位数
	// 把十进制数字转化成二进制字符串，对应转化为布尔数组，例如0x01010变成[false,true,false,true,false]
	var aclStr = (parseInt(acl) & 31).toString(2);
	var aclArr = [];
 ...
 * 增加回调方法
 */
rh.ui.linkSelect.prototype.callBack = function() {
};
/**
 * 增加点击前判断方法
 */
...
rh.ui.linkSelect.prototype.fillData = function() {
	var _self = this;
	var param = {};
	param[UIConst.EXT_WHERE] = " and DATA_ID='" + this.pkCode + "' and SERV_ID='" + this.srcServId + "'";
...
//	}
	jQuery.each(data,function(i,n) {
		var tr = jQuery("<tr></tr>");
		var td1 = jQuery("<td class='ui-linkSelect-default-td  ui-linkselect-default-delete'><a>" +(i+1)+". "+n.TITLE + "</a></td>").appendTo(tr);
...
rh.ui.Date.prototype.setDatePicker = function(config) {
	...
	switch (dateType) {
		case "DATETIME" :
			onClickStr = "WdatePicker({startDate:'%y-%MM-%dd %H:%m:%ss',dateFmt:'yyyy-MM-dd HH:mm:ss',alwaysUseStartDate:false,onpicked:function(){_self.onchange();}";
			break;
		case "DATETIMEH" :
			onClickStr = "WdatePicker({startDate:'%y-%MM-%dd %H',dateFmt:'yyyy-MM-dd HH',alwaysUseStartDate:true,onpicked:function(){_self.onchange();}";
			break;
		case "DATETIMEHM" :
			onClickStr = "WdatePicker({startDate:'%yyyy-%MM-%dd %HH:%mm',dateFmt:'yyyy-MM-dd HH:mm',alwaysUseStartDate:true,onpicked:function(){_self.onchange();}";
			break;
		case "YEAR" :
			onClickStr = "WdatePicker({startDate:'%yyyy',dateFmt:'yyyy',onpicked:function(){_self.onchange();}";
			break;
		case "MONTH" :
			onClickStr = "WdatePicker({startDate:'%yyyy-%MM',dateFmt:'yyyy-MM',alwaysUseStartDate:true,onpicked:function(){_self.onchange();}";
			break;
		case "CUSTOM" :
			onClickStr = "WdatePicker({startDate:'" + configArray[2]
					+ "',dateFmt:'" + configArray[3] + "',alwaysUseStartDate:"
					+ configArray[4] + ",onpicked:function(){_self.onchange();}";
			break;
		case "TIME" :
			onClickStr = "WdatePicker({startDate:'',dateFmt:'H:mm',alwaysUseStartDate:true,onpicked:function(){_self.onchange();}";
			break;
		default :
			onClickStr = "WdatePicker({onpicked:function(){_self.onchange();}";
	}
...
rh.ui.DictChoose = function(options) {
	...
	// 选择
	this._choose = jQuery("<span class='iconChoose " + this.getBackClass() + "' ></span>");
	// 取消选择
	this._cancel = jQuery("<span class='iconCancel " + this._cancelClass + "'></span>");
	// obj容器
	...
	if (data) {
		...
		if (data[this._id + "__STATUS"]) {
			...
			if (data[this._id + "__STATUS"] == UIConst.STR_NO) {
				this._lineStatus.addClass("rh-user-info-offline-status").attr("title", "离线");
			} else if (data[this._id + "__STATUS"] == UIConst.STR_YES) {
				this._lineStatus.addClass("rh-user-info-online-status").attr("title", "在线");
			}
		}
...
rh.ui.Image = function(options){
	...
	this._file = new rh.ui.File({"config":this._opts.config});
...
rh.ui.ItemFile.prototype.setValue = function(val){
	if (val && val.length > 0 && val.indexOf(";") >= 0) {
		...
		if (name.length > size) {// 超过size个字符，则取size的前一半后一半，在title里放置完整的文件名
			var suffix = name.substr(id_name[1].length - size / 2, size / 2);
			name = name.substr(0, size / 2) + "..." + suffix;
		}
		this._obj.find(".fileName").first().html("<a href='/file/" + id_name[0] +  "' title='" + id_name[1] + "' target='_blank'>" + name + "</a>");
		this._input.val(val);
	}
};
/**
...
rh.ui.Upload = function(options) {
	...
	// 按钮样式
	var btnStyles = options.btnStyles?options.btnStyles:".Button {font-family:宋体;font-size:12px;color:#222222}";
	// 按钮图片
 ...
 * 上传之前的事件 
 */
rh.ui.Upload.prototype.beforeUpload = function() {
};
/**
 * 文件成功上传事件
 */
...
rh.ui.Iframe = function(options) {
	...
	this._default = opts._default;
	// 初始化
	var iframeStr = this.expFunc(this._opts.confg, null);
	iframeStr = iframeStr.replace(/<iframe/g, "<iframe id='" + this._opts.id + "' ");
	this.obj = jQuery(iframeStr);
};

/**
 * 设置_parentViewer
 */
rh.ui.Iframe.prototype.afterRender = function() {
	var opts = this._opts;
	//卡片iframe传递卡片句柄
	if (frames[opts.id]) {
...
rh.ui.Form.prototype.validate = function() {
	...
	for (var id in this._validation) {// 校验每一个字段
		...
		if (this.getItem(id)._getValidateObj) {
			// 校验对象
			...
			if (val && (typeof(val)!="object")) {
				...
				if(this.getItem(id).type=="DataService"){
					...
					//判断列表如果需要校验，对列表进行校验
					if(validateGrid.needValidate() && !validateGrid.validate()){
						pass = false;
						continue;
					}
				}
...
rh.ui.Form.prototype.getItemWidth = function(itemCols,cols) {
	...
	var selfTempl = 90;
	if (itemCols >= this.cols) {
		labelWidth = labelWidth / this.cols;
		itemWidth = 100 - (blankWidth * cols);
	} else {
		labelWidth = labelWidth / itemCols;
		itemWidth = 100 * (itemCols / this.cols) - itemCols * blankWidth;
		maxWidth = maxWidth / this.cols * itemCols;
	}
	// 1是留出来的左右边距
...
rh.ui.Form.prototype._createItem = function(data, cols) {
	...
	var width = this.getItemWidth(data.ITEM_CARD_COLS,cols);
	...
	if (cols > 0) {// 系统定义页面
		...
		itemDiv.append(jQuery("<span class='left' style='width:" + labelWidth + "%;'></span>").append(label.obj));
				...
				switch (type) {
					...
					case UIConst.FITEM_ELEMENT_DATA_SERVICE :
						...
						jQuery.each(linkServ, function(i,n) {//生成子功能过滤条件
							...
							if(n.LINK_SERV_ID == data.ITEM_INPUT_CONFIG && n.LINK_SHOW_POSITION==1){
								linkServData = n;
							}
						});
						if(linkServData){
							//获取容器宽度
							var viewWidth = data.ITEM_CARD_WIDTH ? data.ITEM_CARD_WIDTH : 800;
							var containerDiv = jQuery("<div></div>").addClass("ui-dataservice-container");
							var temp = {
								"sId" : data.ITEM_INPUT_CONFIG,
								"pCon" : containerDiv,
								"isReadOnly" : isReadOnly,
								"isHidden" : isHidden,
								"width":viewWidth,
	...
	if (ui) {
		...
		var rightSpan = jQuery("<span class='right' style='width:" + rightWidth + "%;'></span>");
		var itemObj = ui._container?ui._container:ui.obj;
		itemObj.attr("tip", data.ITEM_TIP.replace(/\"/g, "'"));
...
rh.ui.Form.prototype.fillData = function(data) {
	...
	for (var key in this._items) {
		...
		if (item.type == "File") {
			...
			var fileData = FireFly.getCardFile(this._servSrcId, servDataId, key);
...
rh.ui.Form.prototype.getChangedItems = function() {
	...
	for (var key in items) {//遍历所有表单域
		var item = items[key];
		if (item.type == "File") {// Attach类型附件不通过Form提交
			continue;
		}
...
rh.ui.Form.prototype.setNotNull = function(key, bool) {
	if (this.getItem(key)) {// 设置失去焦点时的校验
...
rh.ui.Form.prototype.getGroup = function(itemCode, inputType) {
	if (!inputType) {
		var groups = jQuery("#" + this._servID + "-mainTab").find("#" + itemCode);
		if (groups.length == 0) {
			return null;
		}
		return groups.first();
 ...
 * @param tmpExp 表达式值
 */
rh.ui.Form.prototype.expFunc = function(tmpExp) {
	var _self = this;
	tmpExp = Tools.itemVarReplace(tmpExp,_self._byIdData);//替换字段级变量
	tmpExp = Tools.systemVarReplace(tmpExp);//替换系统变量
	tmpExp = Tools.itemVarReplace(tmpExp,_self._linksVars);//关联定义的变量替换
	tmpExp = tmpExp.replace(/undefined/g,'');//替换undefined
