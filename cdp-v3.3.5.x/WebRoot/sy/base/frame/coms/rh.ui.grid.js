...
rh.ui.grid = function(options) {
	...
	this.endNum = this.startNum + this.showPageNum; // 中间页码的最后一个页码
	//系统变量到临时变量
...
rh.ui.grid.prototype._bldBodyTr = function(num,index,trData) {
	...
	//@TODO:优化
	var pkConf = this.pkConf;
	var pk = trData[pkConf] || "";
	var tr = [];
	var trTemp = [];
	...
	tr.push("<input type='" + _self._checkRadio + "' class='rowIndex' id='href-" + pk + "' ");
	tr.push("indexL='");
	tr.push(index);
	tr.push("'></input>");
	tr.push("</td>");
    ...
    for (j; j < len; j++) {
		...
		if (itemCode == pkConf) {
			...
			tr.push("' style='");
			if (m.ITEM_LIST_WIDTH && m.ITEM_LIST_WIDTH > 0) {
...
rh.ui.grid.prototype._bldPage = function() {
	...
	if(this._buildPageFlag === "false" || this._buildPageFlag === false) {
	...
	} else if (this._lPage.PAGES && (this._lPage.PAGES == "")) {//没有总条数的情况
		...
		// 移动页码
		var movePage = Math.floor(this.showPageNum / 2);
		if(currentPageNum > this.startNum + Math.floor((this.endNum - this.startNum) / 2)) {// 如果点击了后面的页码，则后移
			if(currentPageNum == sumPage) {// 点击了最后一页
				...
				if((this.endNum + movePage) < sumPage) {// 后移3个页码
					this.endNum += movePage;
				} else {
					this.endNum = sumPage;
				}
				
				if((this.startNum + movePage) < sumPage && (this.endNum - this.startNum - movePage == this.showPageNum)) {
					this.startNum += movePage;
				} 
			}
		} else {// 否则前移
			if(currentPageNum == 1) {// 点击了第一页
				...
				if((this.startNum - movePage) > 0) {
					this.startNum -= movePage;
				} else {
					this.startNum = 1;
				}
				
				if((this.endNum - movePage) > 0 && (this.endNum - movePage - this.startNum == this.showPageNum)) {
					this.endNum -= movePage;
				}
			}
		}
		...
		// 显示尾页
		if(sumPage > this.endNum) {
			this._page.append("...").append(jQuery("<a href='javascript:parent.window.scroll(0,0);' class='ui-corner-4'>" + sumPage + "</a>").click(function(){
				_self.lastPage();
			}));
		}
		// 显示下一页
...
rh.ui.grid.prototype._bindTrEvent = function(){
	...
	jQuery(".indexTD",this._table).bind("click",function(event) {
...
//	//鼠标按下事件
//    this.getBodyTr().bind("mousedown",function(event) {
//    	this.x = event.clientX;
//        return false;
//	});
//	//鼠标抬起事件
//    this.getBodyTr().bind("mouseup",function(event) {
...
rh.ui.grid.prototype.dblClick = function(func,parSelf) {
    ...
    this.getBodyTr().bind(_self._clickFeel,function(event) {
    	...
    	if ((node.attr("class") != "rowIndex") && (node.attr("class") != "checkTD")) {
    		...
    		jQuery(".rowIndex", _self.getBodyTr()).removeAttr("checked");
    		jQuery(".rowIndex",pTr).attr("checked","true");
    		pTr.addClass("tBody-selectTr");
...
rh.ui.grid.prototype.trClick = function(func,parSelf) {
	var _self  = this;
    this.getBodyTr().bind("click",function(event) {
		var node = jQuery(event.target);
		var pTr = node.parent();
		_self.getBodyTr().removeClass("tBody-selectTr");
		jQuery(".rowIndex",_self.getBodyTr()).removeAttr("checked");
...
rh.ui.grid.prototype._bldBodyTrModify = function(num,index,trData) {
    ...
    for (j; j < len; j++) {
		...
		if (itemCode == pkConf) {//@TODO:未考虑多主键情况
			//主键列
			tempPK = value;
			tr.push("<td class='");
			tr.push(UIConst.PK_KEY);
			...
			if (!isReadOnly && (m.ITEM_LIST_EDIT == 1 || _self._allTDModify == true)) { 
				...
				if ((index > 0) && ((inputType == UIConst.FITEM_ELEMENT_SELECT) || (inputType == UIConst.FITEM_ELEMENT_RADIO)
					...
					tr.push(_self._bldSelect(m.DICT_ID,trData[linkCode],trData[pkConf],linkCode,notNull));
				    ...
				    tr.push(_self._bldDict(m.DICT_ID,trData[linkCode],value,
				            trData[pkConf],linkCode,itemCode,m.ITEM_INPUT_CONFIG));                   			    
				} else {
					var typText = "text";
					if (inputType == UIConst.FITEM_ELEMENT_PSW) {//密码框
						typText = "password";
