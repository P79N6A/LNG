...
try {
	GLOBAL.defaultFrameHei = parent.document.documentElement.clientHeight - 82;
} catch(e) {
};
GLOBAL.getDefaultFrameHei = function() {//页面高度-头-tab条高度-容差3px
	if (parent.Cookie.get(GLOBAL.cookieBanner) == "hide") {
		return parent.GLOBAL.defaultFrameHei + 45;
	} else {
		return parent.GLOBAL.defaultFrameHei;
	}
};
GLOBAL.defaultLeftMenuWid = 60;
GLOBAL.defaultRightHomeTabsWid = function() {
    var res = 0;
	res = document.documentElement.clientWidth - 60;
	return res;
};
GLOBAL.style = {}; //风格样式
...
function loginJuge(resultData) {
	if(typeof(resultData) == "string"){
		if (resultData.indexOf("var login = \"LOGIN\";") > 0) {
			Tip.showError("系统超时，需要重新登录！",true);
			var temp = resultData.replace(/<script>/g,"").replace(/<\/script>/g,"");
			eval(temp);	
			alert("系统超时，需要重新登录！");
 ...
 * 列表查询调用
 */
function listQueryData(postUrl,dataParams) {
    var resultData = {};
...
var FireFly = {
    ...
    getCardFile: function(sId,dataId,itemCode) {
    ...
    getDict: function(dictCode,pid,extWhere,level,params) {
    ...
    login: function(userCode, pwd, cmpyCode) {
    	var params = {
    		loginName: userCode,
    		password: pwd,
    		cmpyCode: cmpyCode
    	};
    	return FireFly.doAct("SY_ORG_LOGIN","login",params,false); 
    },
    logout: function() {
	 ...
	 * @param ftlFile 打印模板文件，默认名字为服务名
	 */
	doPrint : function(servId, servSrcId, dataId, ftlFile) {
		if (ftlFile) {
			window.open("/sy/comm/print/doPrint.jsp?servId=" + servId + "&servSrcId=" + servSrcId + "&dataId=" + dataId + "&ftlFile=" + ftlFile);
		} else {
			window.open("/sy/comm/print/doPrint.jsp?servId=" + servId + "&servSrcId=" + servSrcId + "&dataId=" + dataId + "&ftlFile=" + servId + ".ftl");
		}
	},
	/**
	 * 跳转到登录页面的前端方法
...
//		    url = getRootPath() + "/";
//		}
		self.parent.window.location.href = FireFlyContextPath + "/"; 
	}
}