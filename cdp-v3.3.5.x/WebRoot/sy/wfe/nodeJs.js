...
WfAction.addAct = function(btnObj){
	...
	var str = "<tr jsonVal='" + jsonVal + "' ><td class='tl'>" + btnObj.name + "</td><td class='tl'>" 
...
function initFileTypeControlWithField(fileTypes, existedFileTypeJson) {
	...
	jQuery.each(fileTypes,function(i,itemold) {
		...
		if(existedFileTypeJson != undefined) {
			jQuery.each(existedFileTypeJson,function(i,existitem) {
				if (item.ID == existitem.ID) {
					existFlag = true;	    
					//构建表格
					var str = "<tr id='"+existitem.ID+"'><td><span>"+item.NAME+"</span></td><td><input type='checkbox' class='read' id='checkRead"+item.NAME+"' value=''></td><td><input type='checkbox' class='write' id='checkWrite"+item.NAME+"' value=''></td><td><input class='addnew' type='checkbox' id='checkAdd"+item.NAME+"' value=''></td><td><input class='del' type='checkbox' id='checkDel"+item.NAME+"' value=''></td></tr>";
		...
		//没找到，设置初始值
		if (!existFlag) {
			var str = "<tr id='"+item.ID+"'><td><span>"+item.NAME+"</span></td><td><input type='checkbox' class='read' id='checkRead"+item.NAME+"' value=''/></td><td><input class='write' type='checkbox' id='checkWrite"+item.NAME+"' value=''/></td><td><input class='addnew' type='checkbox' id='checkAdd"+item.NAME+"' value=''/></td><td><input class='del' type='checkbox' id='checkDel"+item.NAME+"' value=''/></td></tr>";
 ...
 * @param strWhere 查询条件
 */
function  openFieldControlDialog(inputName,strWhere) {
    var servIdName = $("#SERV_ID").val();
	var servPidName = $("#SERV_PID").val();
	var extWhere = " and SERV_ID in (^"+servIdName+"^,^"+servPidName+"^)";
	
	if(strWhere){
		extWhere += strWhere;
	}
	var configStr = "SY_SERV_ITEM,{'TARGET':'"+inputName+"~','SOURCE':'ITEM_CODE~ITEM_NAME','EXTWHERE': '" + extWhere + "','TYPE':'multi'}";
		...
		var configStr = "SY_WFE_PROC_DEF_ACT,{'TARGET':'"+inputName+"~','SOURCE':'ACT_CODE~ACT_NAME','EXTWHERE':'' ,'TYPE':'multi'}";
   		...
   		var options = {"itemCode":inputName,"config" :configStr,"rebackCodes":inputName,"parHandler":this,"formHandler":this,"replaceCallBack":function(result){
			...
			for(var i=0;i < actCodes.length;i++){
				var btnObj = {name:actNames[i],code:actCodes[i],type:"WF"};
				WfAction.addAct(btnObj);
			}
		}};
		var queryView = new rh.vi.rhSelectListView(options);
...
function  openButtonDialog(inputName,formTableName,pTableName) {
		...
		var configStr = "SY_SERV_ACT,{'TARGET':'"+inputName+"~','SOURCE':'ACT_CODE~ACT_NAME','EXTWHERE':'" + extSql + "' ,'TYPE':'multi'}";
		
   		var options = {"itemCode":inputName,"config" :configStr,"rebackCodes":inputName,"parHandler":this,"formHandler":this,"replaceCallBack":function(result){
			var actCodes = result.ACT_CODE.split(",");
			var actNames = result.ACT_NAME.split(",");
			for(var i=0;i < actCodes.length;i++){
				var btnObj = {name:actNames[i],code:actCodes[i],type:"FORM"};
				WfAction.addAct(btnObj);
			}
		}};
		var queryView = new rh.vi.rhSelectListView(options);
...
function openMindTypeCode(inputName,extWhere){
	...
	var options = {"itemCode":inputName,"config" :configStr,"rebackCodes":inputName,"parHandler":this,
			"formHandler":this,"replaceCallBack":function(obj){
				...
				jQuery("#" + inputName + "__NAME").val(obj.CODE_NAME);
			}};		
	var queryView = new rh.vi.rhSelectListView(options);
	queryView.show(null,[50,0]);
}
...
var dataUpdater = {
	...
	addItem : function(moment,conds,field,value) {
		...
		row.push("<option value='MIND'>保存意见</option></select>");
		row.push("</td><td>");
