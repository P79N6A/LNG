...
<%
	...
	*/
	...
	// 审签单数据
	Bean queryBean = new Bean().set(Constant.PARAM_SERV_ID, servId).set(Constant.KEY_ID, dataId);
	...
	*/
	...
	// 拼出模板全路径
	String ftl  = Context.appStr(Context.APP.SYSPATH) + "/" + servPrefix + "/ftl/print/" + ftlFile;
	
	// 如果模板不存在则调用错误页面模板
	boolean ftlExist = new File(ftl).exists();
	if (!ftlExist) {
		bean.set("ftlPath", "/" + servPrefix + "/ftl/print/" + ftlFile);
		ftl = Context.appStr(Context.APP.SYSPATH) + "/sy/print/ERROR.ftl";
	}
