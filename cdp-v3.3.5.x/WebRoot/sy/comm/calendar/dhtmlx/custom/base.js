...
RHTimeCenter._setFrameHei = function(replaceHei) {
	...
	var id = MB.serv + "-show-do-tabFrame";
...
RHTimeCenter._showDetail = function(eventObj){
	...
	if(!eventObj.detailType){
		...
		var options = {"sId": servId,"widHeiArray": MB.widHeiArray,"xyArray": [50,Math.max(parent.document.documentElement.scrollTop, parent.document.body.scrollTop)],
		...
		delete cardView._data.BTNS.deleteEve;// 将缓存中取得的对象属性归零
		cardView._data.BTNS.deleteEve = deleteBtn;
		//展示卡片
		cardView.show();
		cardView.refreshA.hide();
		RHTimeCenter.currDetailObj = cardView;//记录当前打开的详细页容器对象
		if(cardView.getItem("CAL_START_TIME")){
			cardView.getItem("CAL_START_TIME").setValue(RHTimeCenter.convertToDateTime(scheduler.getEventStartDate(eventObj.id)));
		}
		if(cardView.getItem("CAL_END_TIME")){
			cardView.getItem("CAL_END_TIME").setValue(RHTimeCenter.convertToDateTime(scheduler.getEventEndDate(eventObj.id)));
		}
		//添加的情形处理
		if(!eventObj[MB.pk]){
			//追加绑定返回按钮
			cardView.backA.bind("click",{"handler":cardView},function(event){
				//未保存则删除前台的事件
		...
		if(RHTimeCenter.afterSave){
			cardView.afterSave = RHTimeCenter.afterSave(eventObj);
		}else{
			cardView.afterSave = RHTimeCenter._afterSave(eventObj);
		}
...
RHTimeCenter._doDeleteData = function(ev,delBean){
	...
	if(resultData._DATA_ == "1"){
		...
		if(delBean.currDetailObj){
			delBean.currDetailObj.backA.click();
			delete RHTimeCenter.currDetailObj;//归零
		}
	}
};
