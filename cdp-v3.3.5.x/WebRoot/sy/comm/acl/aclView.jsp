...
<style type="text/css">
	...
	#acl_left{float:left;overflow:auto;background-color:white;margin-left:10px;height:530px;border:1px solid #ccc;}
	#acl_right {float:left;overflow:auto;background-color:white;margin-left:10px;height:530px;border:1px solid #ccc;}
</style>
		...
		jQuery("#acl_save").click(function() {// 保存修改
			...
			if(_addedNodes.length > 0) {// 需要添加
				jQuery.each(_addedNodes, function(index, node){
					...
					addedNodes.push({"SERV_ID":SERV_ID,"ACT_CODE":ACT_CODE,"ACL_TYPE":ACL_TYPE,"ACL_OWNER":ACL_OWNER,<%if (cmpyCode.length() > 0) {%>"S_CMPY":"<%=cmpyCode%>",<%}%>"ACL_OTYPE":ACL_OTYPE});
				});
			}
			var resultData;
