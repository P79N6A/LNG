...
<html>
    <head>
        <meta charset="UTF-8" />
        <title>${topic.TOPIC_TITLE!'帖子'}</title>
		<#include "global.ftl"/>
                                        ...
                                        <div>
                                            <div class="avatar">
                                                <div class="qz_feed_avatar f_user_head">
													...
													<img alt="${topic.S_UNAME!topic.S_USER}" src="" id="huati" width="100" height="100">
                                                </div>
                                            </div>
                                            <div class="user_info">
                                                <div class="authi">
                                    ...
                                    <td id="plc_0">
                                        ...
                                        <div class="plc" style="margin-bottom:10px;">
                                            ...
                                            <div class="mtm pbm bbda">
                                                <h1 class="ts"><a id="thread_subject">${topic.TOPIC_TITLE}</a></h1>
                                            </div>
                                            <div class="pi">
                                                ...
                                                <div class="pti">
                                                    ...
                                                    <div class="authi xg1">
                                                        <span id="authorposton0" class="ct" title="${topic.S_MTIME}">发表于 <em id="fabiao"></em></span>
                                                        <span class="pipe">|</span>
                                                      	  查看: ${topic.TOPIC_READ_COUNTER!0}<span class="pipe">|</span>回复: ${topic.COMMENT_COUNTER!0}
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="pct">
											...
											<!-- 投票_begin -->
											<@poll_list debugName="投票" topicId="${topic.TOPIC_ID}">
	...
	<script type="text/javascript">
	    jQuery(document).ready(function(){
	    	/* 阅读+1*/
			top.FireFly.doAct("SY_COMM_BBS_TOPIC","increaseReadCounter",{"TOPIC_ID":"${topic.TOPIC_ID!''}"},false,true);
				
			/* 评论_begin */
		    var opts = {"COMMENT_SERV": "SY_COMM_BBS_TOPIC","SAVE_METHOD": "reply","DATA_ID": "${topic.TOPIC_ID}","SERV_ID":"SY_COMM_BBS_TOPIC", "SHOWNUM":10,"NOWPAGE":1,"pCon":jQuery("#postlist")};
		    var listView = new rh.vi.comment(opts);
			
			//话题修改
			if("${topic.S_USER}"==parent.System.getVar("@USER_CODE@")){
				jQuery("#bbs_modify").parent(".top").css("display","block");
				listView.modifyTopic(jQuery("#bbs_modify"));
			}
			
			//话题人头像显示
			jQuery("#huati").attr("src","/file/${topic.S_USER__IMG}");
			jQuery("#huati").bind("mouseover", function(event){
							 new top.rh.vi.userInfo(event, "${topic.S_USER}");
						});
			//话题发表时间
			var timeago = "";
			if ("${topic.S_MTIME}") {
				timeago = "${topic.S_MTIME}";
