...
<div class="portal-target wp100 floatLeft">
	<#if group="true">
		...
		<@coms_html comsId="GROUP_NEWS_NAV" siteId="${chanel.SITE_ID}" comparam="{'titleBar':'portal-title-none'}">
			${coms.AREA}
		</@coms_html>
		<#else>
		<@coms_html comsId="NEWS_NAV" siteId="${chanel.SITE_ID}" comparam="{'titleBar':'portal-title-none'}">
			${coms.AREA}
		</@coms_html>
	</#if>
</div>
<div class="portal-target wp70 floatLeft">
	<@channel_list debugName="栏目下栏目" channelId="${chanel.CHNL_ID}" SITE_ID="${chanel.SITE_ID}">
...
<div class="portal-target wp30 floatLeft">
	<div class='portal-box'>
		...
		<div class='portal-box-con'>
			<ul class="order">
			<@news_list debugName="新闻资讯排行" SITE_ID="${chanel.SITE_ID}" _ROWNUM_="9"> 
		  	<#list tag_list as news>
		  	<li><a href="javascript:doView('${news.NEWS_ID}','${news.NEWS_SUBJECT}');">
		  		<@text_cut s="${news.NEWS_SUBJECT}" length="18">${text}</@text_cut>
		  	</a></li>
	...
	<div class='portal-box' style="min-height:345px;">
		...
		<div class='portal-box-con'>
		   <@news_list debugName="标题新闻" SITE_ID="${chanel.SITE_ID}" _ROWNUM_="3" NEWS_TYPE="5">
		   <#list tag_list as news>	
