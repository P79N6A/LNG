...
<@news_attach debugName="新闻页面" newsId="${news.NEWS_ID}" picCount="-1" attachCount="-1" pollCount="1">
    	...
    	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="mt5">
			...
			<tr>
				<td style="margin:10px 0;padding:0 1%;align:left;" align="left">当前位置： 
					<a href="/news/channel/${channel.CHNL_ID}/index_1.html?group=${group}">
						${channel.CHNL_NAME!""}
					</a> &gt; 
					${news.NEWS_SUBJECT!""}
				</td>
			...
			<tr>
				<td style="padding-bottom:10px;padding:0 1%;">
					<div class="news_container">
						<div class="news_box">
							<div class="news_title">${news.NEWS_SUBJECT!"没有标题"}</div>
							<div class="news_info">
								<#if (news.NEWS_TIME)??>
									发布日期：${news.NEWS_TIME}&nbsp;&nbsp;
								</#if>
								作者：
								<#if (news.S_USER__NAME)??>
									${news.S_USER__NAME}
									<#else> ${news.S_USER}
								</#if>			
								<#if (news.NEWS_SOURCE)??>
							...
							<div class="news_content">${news.NEWS_BODY!""}</div>
						...
						<!-- 评论 begin-->
						<#if (news.NEWS_COMMENT_STATUS!0)?number != 0>
...
jQuery(document).ready(function(){
	...
	/* 评论_begin */
	(function news(){
		var opts = {"DATA_ID": "${news.NEWS_ID}", "SERV_ID":"SY_COMM_NEWS", "SHOWNUM":10,"NOWPAGE":1,"pCon":jQuery("#comment_show")};
