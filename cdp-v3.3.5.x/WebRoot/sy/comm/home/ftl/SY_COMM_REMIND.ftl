...
<script type="text/javascript">
	...
	<#--对DIV消息删除按钮绑定事件-->
	jQuery("#remindDel").find("span").unbind("click").bind("click",function(event){
		var delID = jQuery("#hiddenID").val();
		jQuery("#"+delID).remove();
		FireFly.listDelete("SY_COMM_REMIND",{"_PK_":delID},false);
		jQuery("#remindSet").hide();
	});
	...
	function showRemindItems(showData){
		...
		var textareaVal = jQuery("<textarea wrap='soft' id='pad_text' style = 'height: 130px;width: 380px;'></textarea>").appendTo(Li_0);
		...
		delTag.unbind("click").bind("click",function(){
			...
			FireFly.listDelete("SY_COMM_REMIND",{"_PK_":showData.remindID},false);
		...
		if(!showData.remindID){
			...
			finishTag.unbind("click").bind("click",function(){
				var remindCount = jQuery("#remindCount").find("textarea").val();
	...
	function createLiFun(dataObj,dataLen,ch){
		for(var i=0;i<dataLen;i++){
			...
			jQuery("<li class='remindLi' id='"+dataObj._DATA_[i].REM_ID+"'>"+
				...
				"<span style='float: left;margin-left: 3%;width: 50%;'>"+dataObj._DATA_[i].REM_CONTENT+"</span>"+
				"</span></li>").appendTo(ul);
		}
	}
</script>