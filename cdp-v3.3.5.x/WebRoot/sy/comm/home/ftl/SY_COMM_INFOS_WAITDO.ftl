<div id='SY_COMM_INFOS_DEAL' class='portal-box ${boxTheme}'>
<script type="text/javascript">
function openKMCard(id,name) {
		var opts = {"tTitle":name,"url":"SY_COMM_INFOS_DEAL.card.do?pkCode=" + id,"menuFlag":3};
		Tab.open(opts);
}
function openKMList() {
		var opts = {"tTitle":"需要我处理的信息","url":"SY_COMM_INFOS_DEAL.list.do","menuFlag":3};
		Tab.open(opts);
}
</script>
...
<td>${content.NEWS_CHECKER__NAME}</td>
<td>${content.NEWS_CHECKED__NAME}</td>
