<style>
	.USER_TXL_back_img{ background:url('/sy/comm/txl/bg.png') no-repeat; }
	#USER_SERCH_TAB_CON .ui-tabs-nav li {background:url('/sy/comm/txl/y.png') repeat-x left -26px;margin:7px 1px 0px 6px;line-height:20px;}
	#USER_SERCH_TAB_CON .ui-tabs-nav li a{background:none;color:black;font-weight:normal;cursor: pointer;}
	#USER_SERCH_TAB_CON .ui-tabs-nav li.ui-tabs-selected {background:url('/sy/comm/txl/y.png') repeat-x;}
	#USER_SERCH_TAB_CON .ui-tabs-nav li.ui-tabs-selected a{background:none;color:white;font-weight:normal;cursor: pointer;}
	#self-resume-tabs .ui-tabs-nav li {background:url('/sy/comm/txl/y.png') repeat-x left -26px;margin:7px 1px 0px 6px;line-height:20px;}
	#self-resume-tabs .ui-tabs-nav li a{background:none;color:black;font-weight:normal;cursor: pointer;}
	#self-resume-tabs .ui-tabs-nav li.ui-tabs-selected {background:url('/sy/comm/txl/y.png') repeat-x;}
...
<div style="background:url('/sy/comm/desk/css/images/rh-back/desk_43.jpg')">
	<div id='USER_TXL' class='portal-box USER_TXL_back_img padding:0' style = 'width:910px;min-height:580px;max-height:580px;border:none'>
	    <div id='USER_SERCH_TAB'  style='height:100%;width:43%;float:left;margin-left:30px;margin-top:7px'>
	        <div id="USER_SERCH_TAB_CON" style='min-height:100%;'>
	            <ul class='portal-box-title' style = 'margin-top:30px'>
	                <li style='margin-left:30px;border-bottom-left-radius:5px;border-top-left-radius:5px;border-right:1px #434544 solid'>
	                    <a href="#USER_SERCH_TAB_PINYIN">拼音结构 </a>
	                </li>
	                <li style='margin-left:-1.5px;'>
	                    <a href="#USER_SERCH_TAB_ORG_TREE">机 构 树</a>
	                </li>
					<li style='margin-left:-1.5px;border-bottom-right-radius:5px;border-top-right-radius:5px'>
	            ...
	            <div id="USER_SERCH_TAB_PINYIN">
					...
					<div style='width: 92%;float:left'>
						<div id="user-search" style="height:35px;">
			            	<input type="text" id="SY_ALL_SEARCH_INPUT" style="height:15px;width:90%;float:left;margin-top:8px;margin-bottom:5px;border-radius:5px;" onkeyup="getSearchDatas(this.value)">
			            	<div id="SY_ALL_SEARCH_BTN" class="btn-search" href="#" style="width:16px;height:16px;float:left;margin-top: 10px;"></div>
			        	</div>
						<div style='height:430px;width:100%;border:1px #cccccc solid;OVERFLOW: auto;' >
				...
				<#--构建组织机构树-->
	            <div id="USER_SERCH_TAB_ORG_TREE"></div>
		...
		<div id='USER_SELF' style='height:480px;width:43%;float:left;margin-left:8%'>
	        <div id='user_all_info' style='width:93%;height:100%;margin-top: 50px;'>
				<div style="height:35%">
					<div id='user_img' style="width:30%;float:left">
						<img id='user_img_view' width='100' height='120'></img>
					</div>
					...
					<div style="width:15%;float:left">
						<ul style='float:left'>
							<li class='user-all-info-lab-li'>性别:</li>
						</ul>
						<ul style='float:left;margin-left:5px'>
							<li class='user-all-info-result-li' id='user_sex'>&nbsp</li>
						</ul>
					</div>
					<div style='position:absolute;margin-left:300px;margin-top:130px'>
				...
				<div style='position:absolute;margin-top: -167px;margin-left: 144px;'>
				...
				<div style="height:60%;">
					...
					<div id='self-resume-tabs' style='min-height:245px;max-height:245px;border:1px #cccccc solid;'>
						 <ul class='portal-box-title' style='background:none '>
			                <li style='border-bottom-left-radius:5px;border-top-left-radius:5px;border-right:1px #434544 solid;border-right:1px #434544 solid'>
			                    <a href="#base-info">基本信息 </a>
			                </li>
			                <li style='margin-left:-1px;border-right:1px #434544 solid'>
			                    <a href="#work-info">工作经历</a>
			                </li>
							<li style='margin-left:-1px;border-right:1px #434544 solid'>
			                    <a href="#education-info">教育经历</a>
			                </li>
							<li style='margin-left:-1px;border-bottom-right-radius:5px;border-top-right-radius:5px;'>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
		 ...
		 */
		var orgOptions = {
            "itemCode": "rh-org-select-serv",
            "config": "SY_ORG_CMPY,{'rtnLeaf':true,'extendDicSetting':{'rhexpand':false,'childOnly':fasle},'TYPE':'single','rtnNullFlag':true}",
            "parHandler": this,
            "hide": "explode",
            "show": "blind",
            "pCon": jQuery("#SERCH_TAB_ORG_TREE"),
            "replaceNodeClick": function(item){
				alert(item.ID);
            }
        };
        var orgDictView = new rh.vi.rhDictTreeView(orgOptions);
        orgDictView.show(event);
	...
	function createSearchUserView(users){
		...
		$(pyzmAry).each(function(){
			var pyzmLi = $("#PY_ZM_LI");
			$("<li class='USER_SERCH_TAB_PINYIN_ZB_li'></li>").text(this).appendTo(pyzmLi);
		});			
		/**
		 * 构建姓名面板区
		 */
		...
		$(".USER_SERCH_TAB_PINYIN_ZB_li").bind("mouseover",function(){
	...
	function showUserInfor(uid){
		...
		if(userObj.DEPT_CODE__NAME&&userObj.DEPT_CODE__NAME.length>0){
			$("#user_cmpy").text(userObj.DEPT_CODE__NAME);
		}else{
	...
	function getUserWorkInfo(Uid){
		var data = {};
		data["_PAGE_"]={"SHOWNUM":'4'};

		var temp = {"sId":"SY_ORG_USER_RESUME_FORWORK","pCon":$('#work-info'),"reset":"false","showPageFlag":true,
			"showSearchFlag":"false","showTitleBarFlag":"false","showButtonFlag":false,"links":data,"extWhere":" and user_code='"+Uid+"'","_SELECT_":"RSM_BEGIN_DATE ||'至'|| RSM_END_DATE JZ_DATE,RSM_COMPANY,RSM_TITLE"};
