...
rh.vi.poll.prototype.show = function() {
	...
	_self._table = jQuery("<table cellspacing='0' cellpadding='4'></table>").attr("id", _self._id).addClass("rh-set-table").appendTo(_self.pollContainer);
	 jQuery.each(options, function(index, item){
		 ...
		 var optTd = jQuery("<td></td>").appendTo(tr);
		 //选项主题
		 if (1 < poll.POLL_MAX_OPTION) { 
		 ...
		 if (showResult) {
	  	 //选项结果图
		 jQuery("<td width='170'><div class='process' style='display: block;'><div class='style"+(index%9)+"' style='width: "+(1.7*rate)+"px;'></div></div></td>").appendTo(tr);
		 //选项结果
		 var optRstTd = jQuery("<td width='110' class='black' id='result'></td>").appendTo(tr);
