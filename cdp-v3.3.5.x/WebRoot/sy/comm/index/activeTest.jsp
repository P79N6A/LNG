...
<div>
	...
	<div id="rh_test_container_ntko" class="rh_test_container">
	    ...
	    <div id="rh_test_zotnClient" style="width:100%;text-align:center;">
	       <div>如果系统没有自动下载安装文件，可以点此链接手动下载安装<a href='/sy/index/file/ruahoClient-huaxia.exe'>系统插件</a>(打包NTKO和ZotnClient的安装文件)</div>
	    </div>
	</div>
...
function browserCheck() {
	if (jQuery.browser.msie && (jQuery("#rh_test_browser").length == 1)) {//ie系列
	    if (jqbrowser.version <= "6.0") {
	        jQuery("#rh_test_browser").append("<div align=center><p class='yellowCon'>系统不支持IE6（或以下）版本的浏览器。请下载并安装 <a href='" + FireFly.getContextPath() + "/sy/index/file/IE8-WindowsXP-x86-CHS.exe' class='rh_test_a'>IE8版本</a></p></div>");
	    } else if (jqbrowser.version == "7.0") {
	        jQuery("#rh_test_browser").append("<div align=center><p class='yellowCon'>您现在使用的是IE7内核版本，将导致系统缓慢，强烈建议您使用较高版本,请下载<a href='" + FireFly.getContextPath() + "/sy/index/file/IE8-WindowsXP-x86-CHS.exe' style='color:red;text-decoration:underline;'>IE8版本</a></p></div>");
	    } else {
        	jQuery("#rh_test_browser").append("<div align=center><p class='greenCon'>您现在使用的是IE" + jqbrowser.version + "版本,可以正常使用系统</p></div>");
	    }
...
if (testBrower()) {
	...
	if (fls.f) {
		...
		jQuery("#rh_test_flash").append("<div align=center><p class='yellowCon'>未安装Flash，将导致系统不能正常运行。请下载并安装 <a href='" + FireFly.getContextPath() + "/sy/index/file/flash.exe' class='rh_test_a'>Flash安装文件</a></p></div>");
	}
	
}
//系统插件测试
function batCheck() {
	jQuery("#rh_test_bat").append("<div align=center><p class='yellowCon'>系统需要添加可信任站点，请先下载此文件并解压后双击执行。<br>请下载并安装 <a href='" + FireFly.getContextPath() + "/sy/index/file/addTrust.zip' class='rh_test_a'>添加可信任站点文件</a>。如已经执行，请忽略。</p></div>");
}
if (testBrower()) {
	batCheck();
}
