...
rh.vi.deskView.prototype._initMainData = function() {
	var _self = this;
	//rh-获取叶子菜单的所有信息
	this.leafMenuModel = parent.Menu.tempLeaf;//全部的叶子服务
	this.deskMenuModel = {};//已显示在桌面上的服务
	this.leftMenuModel = {};//未显示在桌面上的服务
	this.rhTempAlertIconsJson = parent.Menu.alertJson;//设定启用消息提醒的图标
	this.rhTempMinBlockIconsJson = parent.Menu.minBlockJson;//文件夹内的小图标
	//rh-获取当前用户的首页图标定义信息
	...
	if (jQuery.isEmptyObject(portalIcon)) {//初始化时的系统处理
	   var deskShowIcon = parent.Menu.DeskShow;
	   var items = [];
	   var item = {};
	   if(deskShowIcon){
	...
	if (this.normalUlId.length == 0) {
	    this.normalIcons = parent.Menu.normal;
...
rh.vi.deskView.prototype._bldBoxHtml = function() {
	...
	appboxHtml +=               '<div class="deskContent rh-font-size"><input id="desk-initApp" type="checkbox" class="rh-checkbox"/>初始化系统桌面程序(还原为初始默认值)</div>';   
	appboxHtml +=               '<h3 class="deskTitle">主题布局</h3>';
	appboxHtml +=               '<div class="deskContent rh-font-size"><p><input id="desk-initTheme" type="checkbox" class="rh-checkbox"/>还原系统默认主题</p><p><input id="desk-initDesk" type="checkbox" class="rh-checkbox"/>还原系统默认桌面设置</p>';
	...
	var temp = encodeURIComponent(FireFly.getHttpHost() + "/index_mb.jsp?userCode=" + System.getUser("USER_CODE"));
...
rh.vi.deskView.prototype._addModule = function (e, el) {
   ...
   var img = jQuery("<div class='img'><p><img clas='block-backImg' src='css/images/app_rh-icons/" + fixid + ".png' /></p></div>");
    ...
    //重新加载点击分类
     ...
     jQuery("#rhDesk-themeSet").live("mouseup",function(event){
        ...
        jQuery.each(data,function(i,n) {//绑定背景图片点击
        	...
        	systemBlock.on("mouseup",function(event) {
...
rh.vi.deskView.prototype._initTabConBind = function () {
     ...
     //绑定右侧应用,点击事件
     $("#app_list_box ul li").on("mouseup",function(event){
        var obj = $(this).find("a");
...
/*
 * 桌面的搜索图标显示设置  SY_DESK_SEARCHICON
 */
...
rh.vi.deskView.prototype._bldInit = function () {
   ...
   jQuery(document).ready(function(jQuery){
      ...
      //lp 绑定“界面设置”事件   
      var d = '';
      jQuery("#openAppBox").bind("mousedown",function(){
         _self._refixminScreenbtn();
         if(!d){
         	//调用通用组件
            ...
            //重新加载点击分类
            if(jQuery("#app_cate_list ul li a.current").length > 0){
               jQuery("#app_cate_list ul li a.current").trigger("click");
            }
            //如果已经创建过那么就显示且退出
            return;   
         }
